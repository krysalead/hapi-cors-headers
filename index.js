module.exports = function (origin, logger) {
  origin = origin == undefined ? "*" : origin
  logger = logger == undefined ? console : logger

  logger.info("CORS activated with:" + origin);

  return function addCorsHeaders(request, reply) {
    if (!request.headers.origin || (request.headers.origin.indexOf(origin) === -1 && origin !== "*")) {
